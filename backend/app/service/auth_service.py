...
class AuthService:
    ...
    async def register_service(register: RegisterSchema):
        ...
        _users = Users(id=_users_id, email=register.email,password=register.password,person_id=_person_id)
    ...
    @staticmethod
    async def logins_service(login: RegisterSchema):
        _email= await PersonRepository.find_by_email(login.email)
        # print(_email)
